...
echo         Interfaz: http://127.0.0.1:5176
echo         Docs API: http://127.0.0.1:5176/api/docs
echo =======================================================
echo.

:: ── Regenerar Modelfile dinámicamente desde el perfil (C12.4) ──
echo [INFO] Regenerando Modelfile desde perfil y config...
python generate_modelfile.py
if %ERRORLEVEL% NEQ 0 (
    echo [WARN] No se pudo regenerar el Modelfile. Se usara el existente.
)
echo.
...
:: ── Iniciar servidor Fronda en WSL 2 ──
echo [INFO] Inicializando servidor de Fronda 1.0 en Ubuntu WSL 2...
...
echo.
echo [OK] Fronda 1.0 activo. API Docs en http://127.0.0.1:5176/api/docs
echo [OK] Cierra esta ventana para finalizar la sesion.
pause
